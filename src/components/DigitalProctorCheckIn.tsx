                ...
                {proctors
                  ...
                  .map((p) => (
                    <option key={p.id} value={p.id}>
                      {p.assignedRoomCode} (P.{p.assignedPosition}) — {p.name}
                    </option>
                  ))}
              </optgroup>
                ...
                <div className="text-right">
                  {selectedProctor.assignedRoomCode ? (
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-indigo-600 text-white font-bold text-xs shadow-xs">
                      ...
                      {selectedProctor.assignedRoomCode} (P.{selectedProctor.assignedPosition})
                    </span>
                  ) : (
          ...
          {rooms
            .filter((room) => {
              ...
              return (
                ...
                (room.proctor2 && room.proctor2.toLowerCase().includes(q))
              );
            })
            .map((room) => {
              ...
              const rec2 = p2 ? sessionRecords.find((r) => r.proctorId === p2.id) : null;
              ...
              return (
                ...
                >
                  ...
                  <div className="p-2.5 rounded-lg bg-white border border-slate-200 space-y-1.5">
                    <div className="flex items-start justify-between gap-1">
                      <div className="truncate">
                        <span className="text-[9px] uppercase font-bold text-slate-400 block">Pengawas 1</span>
                    ...
                    {rec1?.signatureUrl ? (
                      ...
                      <div className="pt-1">
                        <button
                          type="button"
                          onClick={() => p1 && handleQuickSelectProctor(p1.id)}
                          className="w-full py-1 px-2 rounded bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-[11px] font-bold transition-colors flex items-center justify-center gap-1"
                        >
                          <PenTool className="w-3 h-3" />
                          <span>Input TTD Pengawas 1</span>
                        </button>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-2.5 rounded-lg bg-white border border-slate-200 space-y-1.5">
                    <div className="flex items-start justify-between gap-1">
                      <div className="truncate">
                        <span className="text-[9px] uppercase font-bold text-slate-400 block">Pengawas 2</span>
                        <div className="text-xs font-bold text-slate-900 truncate">
                          {p2?.name || room.proctor2 || '(Belum diset)'}
                        </div>
                      </div>
                      {rec2 ? (
                        <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-700 text-[10px] font-bold border border-emerald-200">
                    ...
                    {rec2?.signatureUrl ? (
                      <div className="flex items-center justify-between pt-1 border-t border-dashed border-slate-100">
                        ...
                        <button
                          type="button"
                          onClick={() => p2 && handleQuickSelectProctor(p2.id)}
                          className="text-[10px] text-indigo-600 hover:text-indigo-800 font-semibold"
                        >
                          Ubah TTD
                        </button>
                      ...
                      <div className="pt-1">
                        ...
                        >
                          <PenTool className="w-3 h-3" />
                          <span>Input TTD Pengawas 2</span>
                        </button>
                      </div>
                    )}
                  </div>
                    ...
                    <td className="py-2 px-3">
                      {rec.roomCode ? (
                        <span className="font-semibold text-slate-800">
                          {rec.roomCode} {rec.position ? `(Pengawas ${rec.position})` : ''}
                        </span>
                      ) : (
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4 border border-slate-200 animate-in zoom-in-95">
            ...
            <div className="text-xs text-slate-600 space-y-1 border-t border-slate-100 pt-3">
              <div className="flex justify-between">
                <span className="text-slate-400">Ruang &amp; Posisi:</span>
                <span className="font-semibold text-slate-800">
                  {previewModalRecord.roomCode || 'Cadangan'} {previewModalRecord.position ? `(Posisi ${previewModalRecord.position})` : ''}
                </span>
              </div>
              <div className="flex justify-between">
