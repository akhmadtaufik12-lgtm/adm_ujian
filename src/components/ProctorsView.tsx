...
export const ProctorsView: React.FC<ProctorsViewProps> = ({
  ...
  // Digital Proctor Attendance Records with localStorage Persistence
  ...
  const assignedProctors = proctors.filter((p) => p.assignedRoomId).length;
  const reserveProctors = proctors.filter((p) => p.role === 'Pengawas Cadangan' || !p.assignedRoomId).length;
  const fullyCoveredRooms = rooms.filter((r) => {
    const p1 = proctors.some((p) => p.assignedRoomId === r.id && p.assignedPosition === 1);
    const p2 = proctors.some((p) => p.assignedRoomId === r.id && p.assignedPosition === 2);
    return p1 && p2;
  }).length;
  ...
  const filteredProctors = proctors.filter((p) => {
    ...
    const matchesRoom = filterRoom === 'all' || 
      (filterRoom === 'unassigned' ? !p.assignedRoomId : p.assignedRoomId === filterRoom);
  ...
  const handleOpenAddModal = () => {
    setFormData({
      ...
      assignedRoomCode: rooms[0]?.roomCode || '',
      assignedPosition: 1,
    });
  ...
  const handleOpenEditModal = (proctor: Proctor) => {
    ...
    setFormData({
      ...
      phone: proctor.phone || '',
      assignedRoomId: proctor.assignedRoomId || '',
      assignedRoomCode: proctor.assignedRoomCode || '',
      assignedPosition: proctor.assignedPosition || 1,
    });
  };
  ...
  const handleSaveSubmit = (e: React.FormEvent) => {
    ...
    const matchedRoom = rooms.find((r) => r.id === formData.assignedRoomId);
    const payload = {
      ...formData,
      assignedRoomCode: matchedRoom ? matchedRoom.roomCode : '',
    };

    if (editingProctor) {
      onUpdateProctor({ ...payload, id: editingProctor.id });
  ...
  const handleAutoAssign = () => {
    ...
    // Sort and assign sequentially to rooms
    const updated = proctors.map((p) => ({ ...p }));
    let pIdx = 0;

    rooms.forEach((room) => {
      if (pIdx < updated.length) {
        updated[pIdx].assignedRoomId = room.id;
        updated[pIdx].assignedRoomCode = room.roomCode;
        updated[pIdx].assignedPosition = 1;
        updated[pIdx].role = 'Pengawas Ruang';
        pIdx++;
      }
      if (pIdx < updated.length) {
        updated[pIdx].assignedRoomId = room.id;
        updated[pIdx].assignedRoomCode = room.roomCode;
        updated[pIdx].assignedPosition = 2;
        updated[pIdx].role = 'Pengawas Ruang';
        pIdx++;
      }
    });

    // Mark remaining as Pengawas Cadangan
    for (; pIdx < updated.length; pIdx++) {
      updated[pIdx].assignedRoomId = undefined;
      updated[pIdx].assignedRoomCode = undefined;
      updated[pIdx].assignedPosition = undefined;
      updated[pIdx].role = 'Pengawas Cadangan';
    }

    onSyncRoomsWithProctors(updated);
  };

  const handleInlineRoomChange = (proctorId: string, roomId: string, position: 1 | 2 = 1) => {
    const matchedRoom = rooms.find((r) => r.id === roomId);
    const updated = proctors.map((p) => {
      if (p.id === proctorId) {
        return {
          ...p,
          assignedRoomId: roomId || undefined,
          assignedRoomCode: matchedRoom ? matchedRoom.roomCode : undefined,
          assignedPosition: roomId ? position : undefined,
          role: (roomId ? 'Pengawas Ruang' : 'Pengawas Cadangan') as Proctor['role'],
        };
      }
      return p;
    });
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs no-print">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Kelola master nama pengawas ruang, plotting tugas otomatis, dan cetak format daftar hadir presensi pengawas resmi.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="bg-slate-100 p-1 rounded-lg flex items-center gap-1 border border-slate-200">
              ...
              >
                ...
                <span>Data &amp; Plotting Pengawas</span>
              </button>
              <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 pt-4 border-t border-slate-100">
          ...
          <div className="bg-indigo-50/70 border border-indigo-100 rounded-lg p-2.5">
            <span className="text-[10px] uppercase font-bold tracking-wider text-indigo-700">Bertugas di Ruang</span>
            <div className="text-lg font-black text-indigo-950 mt-0.5">{assignedProctors} Orang</div>
          </div>
          <div className="bg-amber-50/70 border border-amber-100 rounded-lg p-2.5">
            <span className="text-[10px] uppercase font-bold tracking-wider text-amber-700">Pengawas Cadangan</span>
            <div className="text-lg font-black text-amber-950 mt-0.5">{reserveProctors} Orang</div>
          </div>
          <div className="bg-emerald-50/70 border border-emerald-100 rounded-lg p-2.5">
            <span className="text-[10px] uppercase font-bold tracking-wider text-emerald-700">Ruang Terisi Penuh (2 Org)</span>
            <div className="text-lg font-black text-emerald-950 mt-0.5">{fullyCoveredRooms} / {rooms.length} Ruang</div>
          </div>
        </div>
      </div>

      {/* SUB-TAB 1: DATA & PLOTTING PENGAWAS */}
      {activeSubTab === 'list' && (
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                title="Bagi pengawas ke Ruang 01 s/d selesai secara berpasangan"
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-md transition-colors cursor-pointer"
              >
                <Shuffle className="w-3.5 h-3.5" />
                <span>Plotting Otomatis ke Ruang</span>
              </button>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-8 pr-3 py-1.5 text-xs border border-slate-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:outline-none w-44 sm:w-56"
                />
              </div>
              ...
              >
                ...
                <option value="unassigned">Belum Ada Ruang (Cadangan)</option>
                {rooms.map((r) => (
                  <option key={r.id} value={r.id}>{r.roomCode} ({r.name})</option>
                ))}
              </select>
            </div>
          </div>

          {/* Table List of Proctors */}
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-slate-700 font-bold uppercase tracking-wider text-[10px]">
                    ...
                    <th className="py-2.5 px-3">Status / Jabatan</th>
                    <th className="py-2.5 px-3">Ruang Tugas (Plotting)</th>
                          ...
                          <div className="max-w-md mx-auto space-y-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-500 mt-1">
                                Anda dapat mengunggah file Excel data guru atau mengimpor salinan spreadsheet secara instan.
                              </p>
                            </div>
                    ...
                    filteredProctors.map((p, idx) => {
                      return (
                        <tr key={p.id} className="hover:bg-slate-50/80 transition-colors">
                          ...
                          <td className="py-2.5 px-3">
                            {/* Inline Room Assignment Dropdown */}
                            <select
                              value={p.assignedRoomId || ''}
                              onChange={(e) => handleInlineRoomChange(p.id, e.target.value, p.assignedPosition || 1)}
                              className="px-2 py-1 text-xs border border-slate-300 rounded font-medium focus:ring-1 focus:ring-indigo-500 bg-white"
                            >
                              <option value="">(Cadangan / Belum Ditugaskan)</option>
                              {rooms.map((r) => (
                          ...
                          <td className="py-2.5 px-3">
                            {p.assignedRoomId ? (
                              <select
                                value={p.assignedPosition || 1}
                                onChange={(e) => handleInlineRoomChange(p.id, p.assignedRoomId || '', Number(e.target.value) as 1 | 2)}
                                className="px-2 py-1 text-xs border border-slate-300 rounded font-semibold bg-white"
                              >
                                <option value={1}>Pengawas 1</option>
                                <option value={2}>Pengawas 2</option>
                              </select>
                            ) : (
          ...
          <div className="bg-white border border-slate-300 rounded-xl p-6 sm:p-8 shadow-xs font-sans text-slate-900 page-break-after-always">
            ...
            <table className="w-full border-collapse border border-slate-900 text-xs mb-4">
              ...
              <tbody className="divide-y divide-slate-900">
                {rooms.map((room, rIdx) => {
                  const p1 = proctors.find((p) => p.assignedRoomId === room.id && p.assignedPosition === 1) 
                    || { id: '', name: room.proctor1 || '...........................................', nip: '-', subject: '-' };
                  const p2 = proctors.find((p) => p.assignedRoomId === room.id && p.assignedPosition === 2)
                    || { id: '', name: room.proctor2 || '...........................................', nip: '-', subject: '-' };
                  ...
                  const rec2 = attendanceRecords.find(
                    (r) => (r.scheduleId === currentSchedule.id || r.subject === currentSchedule.subject) &&
                           ((p2.id && r.proctorId === p2.id) || (p2.name && p2.name !== '-' && r.proctorName.includes(p2.name)))
                  );
                  ...
                  return (
                    <React.Fragment key={room.id}>
                      {/* Row 1: Pengawas 1 */}
                      <tr>
                        <td rowSpan={2} className="border border-slate-900 py-2 px-2 text-center font-bold">
                          {rIdx + 1}
                        </td>
                        <td rowSpan={2} className="border border-slate-900 py-2 px-2 text-center font-black bg-slate-50">
                      ...
                      <tr>
                        ...
                        <td className="border border-slate-900 py-1.5 px-2 text-center font-mono text-slate-800 text-[11px]">
                          {showDigitalSignatures && rec2?.checkInTime ? rec2.checkInTime : '...... : ......'}
                        </td>
                        <td className="border border-slate-900 py-1 px-3 text-right">
                          {showDigitalSignatures && rec2?.signatureUrl ? (
                            <div className="flex items-center justify-end gap-1.5 py-0.5">
                              <img 
                                src={rec2.signatureUrl} 
                                alt="TTD" 
                                className="h-8 w-auto max-w-[85px] object-contain inline-block" 
                              />
                ...
                {reserveProctors > 0 && (
                  <>
                    ...
                    {proctors
                      .filter((p) => p.role === 'Pengawas Cadangan' || !p.assignedRoomId)
                      .slice(0, 4)
                      .map((cad, cIdx) => {
            ...
            <form onSubmit={handleSaveSubmit} className="p-5 space-y-3.5 text-xs">
              ...
              <div className="border-t border-slate-200 pt-3">
                <span className="block text-[11px] font-bold text-slate-700 mb-2">
                  Penugasan Ruang (Opsional):
                </span>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] text-slate-600 mb-1">Pilih Ruang:</label>
                    <select
                    ...
                    >
                      <option value="">(Belum / Cadangan)</option>
                      {rooms.map((r) => (
                        <option key={r.id} value={r.id}>{r.roomCode} - {r.name}</option>
                      ))}
                    </select>
                  ...
                  <div>
                    <label className="block text-[10px] text-slate-600 mb-1">Posisi Tugas:</label>
                    <select
                      value={formData.assignedPosition || 1}
                      onChange={(e) => setFormData({ ...formData, assignedPosition: Number(e.target.value) as 1 | 2 })}
                      className="w-full px-2.5 py-1.5 border border-slate-300 rounded-md bg-white font-medium"
                    >
                      <option value={1}>Pengawas 1 (Utama)</option>
                      <option value={2}>Pengawas 2 (Pendamping)</option>
                    </select>
                  </div>
                </div>
              </div>
